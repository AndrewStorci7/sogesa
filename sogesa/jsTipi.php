	...
	$(document).ready(function() {
		jQuery.post('viewTipi.php',  {tipo: tipo,pager: pager}, viewList, 'json');
		jQuery.post('pageTipi.php',  {tipo: tipo,pager: pager}, pagerView, 'json');
	});
	...
	function viewList(data){
					...
					{
						x= x+'<tr id='+objChild[i].id+'><td>'+objChild[i].tipo+'</td><td>'+objChild[i].desc+'</td><td>'+'<a href="updTipo.php?id='+objChild[i].id+'"><img style="width:40px; height:auto" title="Modifica" src="edit.png"></a>'+'</td><td>'+'<a href="javascript:void(0);" onClick="javascript:clickFn('+objChild[i].id+');"><img style="width:40px; height:auto" title="Elimina" src="delete.png"></a>'+'</td></tr>';
					}
					x=x+'</table>';
					}
		$("#risultato").html(x);
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					...
					if(cntFile!=0){
						...
						alert("Prima di eliminare questo tipo, bisogna eliminare gli eventi associati.");
					}
			}
			}
			xhttp.open("GET", "delTipo.php?id="+id, true);
			 xhttp.send();
			};
		</script>
