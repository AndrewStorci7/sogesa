  ...
  <?php
	...
	$y="SELECT * FROM partecipanti ".$whereQuery." ORDER BY idPartecipante DESC";
...
while($row = mysqli_fetch_array($result) ){
	...
	if((trim($marca)!="")||(trim($modello)!="")||(trim($disciplina)!="")){
		if(trim($marca)!=""){
			$yy="SELECT * FROM mezzi WHERE idPartecipante=".$id." AND marca LIKE '%".$marca."%'";
		...
		if(trim($modello)!=""){
			$yy="SELECT * FROM mezzi WHERE idPartecipante=".$id." AND modello LIKE '%".$modello."%'";
			$resulty = mysqli_query($conn,$yy);
		...
		if(trim($disciplina)!=""){
			$yy="SELECT * FROM mezzi WHERE idPartecipante=".$id." AND disciplina LIKE '%".$disciplina."%'";
			$resulty = mysqli_query($conn,$yy);
	...
	if($ctrlItemId==1){
		$xxxxx=$x+10;
		if(($controllo>=$x)&&($controllo<$xxxxx)){
