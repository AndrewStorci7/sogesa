  ...
  <?php
	...
	$y="SELECT * FROM tipieventi WHERE IdTipoE=".$tipo."";
...
$pager=1;
	...
	$y="SELECT * FROM partecipanti ".$whereQuery." ORDER BY Cognome ASC";
...
while($row = mysqli_fetch_array($result) ){
	...
	if($ctrlItemId==1){
		...
		$email=html_entity_decode($email,ENT_QUOTES,"UTF-8");
			...
			if($num_rowsy>0){
				...
				while($rowy = mysqli_fetch_array($resulty) ){
					...
					$ctrlMezzook=1;
					if($eventilist>0){
						$yyCtrlm="SELECT * FROM scarichi WHERE idMezzo='".$idM."' AND codEv='".$eventilist."' AND codPar='".$id."'";
						$resultyCtrlm = mysqli_query($conn,$yyCtrlm);
						$num_rowsyCtrlm = mysqli_num_rows($resultyCtrlm);
						if($num_rowsyCtrlm==0){
							$ctrlMezzook=0;
						}
					}
					if($ctrlMezzook==1){
						...
						$marcay = $rowy['marca'];
						$modelloy = $rowy['modello'];
						$disciplinay = $rowy['disciplina'];
						$cilindratay = $rowy['cilindrata'];
						$massay = $rowy['massa'];




						$autoutente.=$marcay.' '.$modelloy.' '.$disciplinay.' '.$cilindratay.' '.$massay;
						$autoutente=html_entity_decode($autoutente,ENT_QUOTES,"UTF-8");
		...
		$jsonResult.='{"id":'.$id.',"nome":"'.$nome.'","cognome":"'.$cognome.'","cf":"'.$cf.'","nascita":"'.$nascita.'","luogo":"'.$luogo.'","citta":"'.$citta.'", "telefono":"'.$telefono.'", "email":"'.$email.'", "tipologia":"'.$tipologia.'", "marca":"'.$marca.'", "modello":"'.$modello.'", "disciplina":"'.$disciplina.'","autoutente":"'.$autoutente.'"}';
	}
}
